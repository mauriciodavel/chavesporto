...
exports.getAllInstructors = async (req, res) => {
  try {
    const { data: instructors, error } = await supabase
      .from('instructors')
      .select('id, matricula, name, email, technical_area, role, created_at')
      .eq('role', 'instructor')
...
exports.createInstructor = async (req, res) => {
  try {
    const { matricula, name, email, password, technicalArea } = req.body;
    ...
    const { data: instructor, error } = await supabase
      ...
      .select('id, matricula, name, email, technical_area, role')
      .single();
...
exports.updateInstructor = async (req, res) => {
  try {
    const { id } = req.params;
    const { name, email, technicalArea, password } = req.body;
